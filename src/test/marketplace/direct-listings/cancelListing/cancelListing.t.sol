...
contract CancelListingTest is BaseTest, IExtension {
    // Target contract
    ...
    function test_cancelListing_whenListingDoesntExist() public {
        vm.prank(seller);
    ...
    function test_cancelListing_whenCallerNotListingCreator() public whenListingExists {
    ...
    function test_cancelListing_success() public whenListingExists whenCallerIsListingCreator {
        vm.warp(listingParams.startTimestamp + 1);

        assertEq(uint8(DirectListingsLogic(marketplace).getListing(listingId).status), uint8(1)); // CREATED

        vm.prank(seller);
        vm.expectEmit(true, true, true, true);
        emit CancelledListing(seller, listingId);
        DirectListingsLogic(marketplace).cancelListing(listingId);

        assertEq(uint8(DirectListingsLogic(marketplace).getListing(listingId).status), uint8(3)); // CANCELLED
    }
}
