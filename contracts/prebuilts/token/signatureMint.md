...
This is a live document that explains the 'signature minting' mechanism used in [egiftcard](https://egiftcard.cc/) `Token` smart contracts.
...
The 'signature minting' mechanism used in [egiftcard](https://egiftcard.cc/) `Token` smart contracts is a way for a contract admin to
...
The egiftcard `Drop` contracts serve the cases described in 2(i).

The egiftcard `Token` contracts serve the cases described in (1) and 2(ii). And the 'signature minting' mechanism is particularly designed to serve the case described in 2(ii).
