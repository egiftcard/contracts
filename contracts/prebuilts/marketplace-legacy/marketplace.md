...
This is a live document that explains what the [egiftcard](https://egiftcard.cc/) `Marketplace` smart contract is, how it works and can be used, and why it is written the way it is.

The document is written for technical and non-technical readers. To ask further questions about `Marketplace`, please join the [egiftcard discord](https://discord.gg/egiftcard) or create a [github issue](https://github.com/egiftcard/contracts/issues).

---
## Background

The [egiftcard](https://egiftcard.cc/) `Marketplace` is a market where where people can sell NFTs — [ERC 721](https://eips.ethereum.org/EIPS/eip-721) or [ERC 1155](https://eips.ethereum.org/EIPS/eip-1155) tokens — at a fixed price ( what we'll refer to as a "Direct listing"), or auction them (what we'll refer to as an "Auction listing").
...
Auctions on egiftcard's Marketplace are [english auctions](https://www.wallstreetmojo.com/english-auction/).
...
The previous (v1) [egiftcard Market contract](https://github.com/egiftcard/contracts/tree/v1) has the following critical pitfalls -
...
These are features that are already offered by popular marketplaces like [OpenSea](https://opensea.io/). The current egiftcard [Marketplace](https://github.com/egiftcard/contracts/blob/main/contracts/marketplace/Marketplace.sol) contract consolidates all these features into a single smart contract, so egiftcard's users can *truly* have their own OpenSea and more.
...
There are two groups of users — (1) egiftcard's customers who'll set up the marketplace, and (2) the end users of egiftcard customers' marketplaces.

To egiftcard customers, the `Marketplace` can be set up like any of the other egiftcard contract (e.g. 'NFT Collection') through the egiftcard dashboard, the egiftcard SDK, or by directly consuming the open sourced marketplace smart contract.

To the end users of egiftcard customers, the experience of using the marketplace will feel familiar to popular marketplace platforms like OpenSea, Zora, etc. The biggest difference in user experience will be that performing any action on the marketplace requires gas fees.

- Egiftcard's customers
  - Deploy the marketplace contract like any other egiftcard contract.
  ...
  - End users of egiftcard customers
...
All events emitted by the contract, as well as when they're emitted, can be found in the interface of the contract, [here](https://github.com/egiftcard/contracts/blob/main/contracts/interfaces/marketplace/IMarketplace.sol). In general, events are emitted whenever there is a state change in the contract.
...
We're working on breaking up, sizing down and optimizing the `Marketplace` contract to accommodate such off-chain actions, and coming up with a central order-book infrastructure that each egiftcard customer can run on their own.
